...
public TicTacToe() {
	...
	panel1.add(new JLabel("Line Thickness: "));
	panel1.add(slider = new JSlider(SwingConstants.HORIZONTAL, 1, 25, 5));
	slider.setMajorTickSpacing(1);
	slider.setPaintTicks(true);
	slider.addChangeListener(this);
	
	panel1.add(oButton = new JButton("O color: "));
	panel1.add(xButton = new JButton("X color: "));
...
private class Board extends JPanel implements MouseListener {
	...
	public void newGame(char winner) {
		...
		if (winner == O) {
			...
			losses++;
			result = "I won!!! Hahahahahaha";
		}
		else {
			draws++;
			result = "It's a draw!";
		}
		if (JOptionPane.showConfirmDialog(null, "You have " + wins + " wins, " + losses + " losses, and " + draws + " draws. \n"
				+ "Do you want to play again?", result, JOptionPane.YES_NO_OPTION) != JOptionPane.YES_OPTION) {
			System.exit(0);
		}
		for (int j = 0; j < 9; ++j) {
			position[j] = BLANK;
